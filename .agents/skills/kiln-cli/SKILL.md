...
- If a requested power stop reports `out_of_memory_while_stopping`, the stopped
  target was reached after an OOM kill, but the desired state alone does not
  prove the stop caused it. The process may not have shut down gracefully;
  increase the memory limit before the next start or use the server's console
  stop command.
