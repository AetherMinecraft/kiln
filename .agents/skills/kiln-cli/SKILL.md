...
- If a requested power stop reports `out_of_memory_while_stopping`, the stopped
  target was reached but the process did not shut down gracefully. Increase the
  memory limit before the next start or use the server's console stop command.
