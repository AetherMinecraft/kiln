...
and ignored by the Ember. Quoted values such as `-Dmessage="hello world"` stay a
single argument. Minecraft server Bricks also get `--nogui` from
`KILN_SERVER_ARGS` unless the recipe clears that variable.
